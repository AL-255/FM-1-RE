...
disassembling `app.bin` and indexing 2062 call-target-derived function entries.
...
| `01-hardware-map.md` | SoC, memory map, SFR regions, and observed peripherals |
...
| `../04-toolchain-and-vendoring.md` | JieLi toolchain and component provenance |
...
2. **Function DB** — the vendor listing yields 2062 flash function entries and
   42 additional RAM/ROM targets. Objdump's `<_fw+0x..>` annotations provide
   direct-call and data/string cross-references (`analysis/function_db.json`,
   `master_*.json`).
3. **Auto-labelling** — exact signature matches against the toolchain
   `libc/libm/compiler-rt` name the standard library functions.
4. **Classification** — reviewed functions were assigned a subsystem and
   one-line purpose; unresolved or locality-inferred entries remain explicitly
   marked. Results are aggregated in `09-function-index.md`.

## Confidence & limits

- Recorded addresses, sizes, direct calls, and byte-identical reassembly are
  exact for the checked-in V13 image; function boundaries remain
  call-target-derived.
