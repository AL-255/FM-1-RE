...
Reuses the original shard boundaries (`analysis/raw-shards/shard_*.txt`). For each shard
range [start,end) emit analysis/shards/shard_<start>_<end>.txt with:
...
def main():
    ...
    shard_files = sorted(glob.glob(os.path.join(ROOT, "analysis", "raw-shards", "shard_*.txt")))
    manifest = []
    # Gap shards: entries not covered by the original raw shard set.
    covered = set()
