...
Dexed::Dexed(uint8_t maxnotes, uint32_t rate)
 : voices{nullptr},
   ...
   max_notes{maxnotes},
   engineMsfa{nullptr}
#if !defined(TEENSYDUINO) && !defined(DEXED_NO_COMPRESSOR)
   , compressor{nullptr}
#endif
   {
  Exp2::init();
  ...
  for (uint16_t i = 0; i < n_samples; i += _N_)
  {
    ...
    for (uint8_t note = 0; note < used_notes; note++)
    {
      if (voices[note].live)
      {
        ...
        for (uint8_t j = 0; j < _N_; ++j)
        {
          buffer[i + j] += signed_saturate_rshift(audiobuf.get()[j] >> 4, 24, 9) / 32768.0f;
          audiobuf.get()[j] = 0;
        }
      }
    }
...
void Dexed::keydown(uint8_t pitch, uint8_t velo) {
  ...
  velo=uint8_t((float(velo)/127.0f)*velocity_diff+0.5f)+velocity_offset;
...
void Dexed::setMasterTune(int8_t mastertune)
{
  ...
  controllers.masterTune = (int(mastertune / 100.0f * 0x4000) << 11) * (1.0f / 12.0f);
}
