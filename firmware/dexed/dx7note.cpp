...
int32_t osc_freq(int midinote, int mode, int coarse, int fine, int detune) {
  ...
  if (mode == 0) {
    ...
    FRAC_NUM detuneRatio = 0.0209f * EXP_FUNC(-0.396f * (((float)logfreq) / (1 << 24))) / 7;
    ...
    if (fine) {
      ...
      logfreq += (int32_t)floorf(24204406.323123f * LOG_FUNC(1 + 0.01f * fine) + 0.5f);
    }
  ...
  for (int op = 0; op < 6; op++) {
    ...
    if (!(ctrls->opSwitch & (1 << op)))  {
      ...
      if (ampmodsens_[op] != 0) {
        ...
        //uint32_t pt = exp(((float)sensamp) / 262144 * 0.07f + 12.2f);
        uint32_t pt = EXP_FUNC(((float)sensamp) / 262144 * 0.07f + 12.2f);
