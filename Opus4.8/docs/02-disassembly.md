...
Artifacts:
- `decomp/app_pi32v2_linear.asm` — full linear listing (`addr  bytes  mnemonic`).
- `decomp/app_pi32v2_annotated.asm` — same, with `FUNC_xxxx (called Nx)` headers.
...
Outputs:
- `decomp/functions_ranked.csv` — entries ranked by inbound call count.
- `decomp/function_entries.txt` — entries with prologue flag + first instruction.
...
cases — produces `reasm/app_reassembled.bin`, which is **byte-for-byte identical**
to the original:
