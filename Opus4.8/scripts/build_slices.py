...
os.makedirs("analysis/funcs", exist_ok=True)
SHARD = 40
shard_idx = []
for si in range(0, len(real), SHARD):
    chunk = real[si:si+SHARD]
    fn = f"analysis/funcs/shard_{chunk[0]:08x}_{chunk[-1]:08x}.txt"
...
print(f"wrote {len(shard_idx)} shards covering {len(real)} functions into analysis/funcs/")
